...
protocol UnifiedProfileServiceProtocol {
    // Personal profiles (olfactive)
    func fetchPersonalProfiles(userId: String, language: String) async throws -> [UnifiedProfile]
    func savePersonalProfile(_ profile: UnifiedProfile, userId: String, language: String) async throws
    func updatePersonalProfile(_ profile: UnifiedProfile, userId: String, language: String) async throws
    func deletePersonalProfile(_ profile: UnifiedProfile, userId: String, language: String) async throws
    func updatePersonalProfilesOrder(_ profiles: [UnifiedProfile], userId: String, language: String) async throws
...
/// Reemplaza OlfactiveProfileService y GiftProfileService
actor UnifiedProfileService: UnifiedProfileServiceProtocol {
    ...
    private let personalProfilesPath = "olfactive_profiles"  // Legacy path for personal profiles
    private let giftProfilesPath = "giftProfiles"            // Legacy path for gift profiles
    ...
    private func personalProfilesRef(userId: String, language: String) -> CollectionReference {
    ...
    func fetchPersonalProfiles(userId: String, language: String) async throws -> [UnifiedProfile] {
        let cacheKey = "unified_personal_profiles_\(userId)_\(language)"
        ...
        let collectionRef = personalProfilesRef(userId: userId, language: language)
        ...
        // Try to decode as UnifiedProfile first, then fall back to OlfactiveProfile
        ...
        for document in snapshot.documents {
            ...
            if data["profileType"] != nil {
                ...
                // Legacy OlfactiveProfile - convert
        ...
        #if DEBUG
        print("✅ [UnifiedProfileService] Personal profiles loaded from Firebase: \(profiles.count)")
        #endif
    ...
    func savePersonalProfile(_ profile: UnifiedProfile, userId: String, language: String) async throws {
        ...
        await invalidatePersonalProfilesCache(userId: userId, language: language)
    }
    ...
    func updatePersonalProfile(_ profile: UnifiedProfile, userId: String, language: String) async throws {
        ...
        try await personalProfilesRef(userId: userId, language: language)
            .document(profileId)
    ...
    private func invalidatePersonalProfilesCache(userId: String, language: String) async {
        let cacheKey = "unified_personal_profiles_\(userId)_\(language)"
